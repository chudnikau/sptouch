...
type
  ...
  TbsDrawElement = set of (daMonth, daDay);

  TbsSkinMonthCalendar = class(TbsSkinPanel)
  private
    FDrawElement: TbsDrawElement;
    procedure SetDrawElement(const Value: TbsDrawElement);
  protected
  ...
  published
    ...
    //chudnikau
    property DrawElement : TbsDrawElement read FDrawElement write SetDrawElement;
  end;
...
procedure TbsSkinMonthCalendar.SetDrawElement(const Value: TbsDrawElement);
begin
  FDrawElement := Value;
  FBtns[0].Visible := daMonth in FDrawElement;
  FBtns[1].Visible := daMonth in FDrawElement;
  FBtns[2].Visible := daMonth in FDrawElement;
  FBtns[3].Visible := daMonth in FDrawElement;
end;
...
var
  ...
  ADay, DayNum: Integer;
begin
  //if daNone in FDrawElement then Exit;
  R := Rect(0, 0, Width, Height);
  AdjustClientRect(R);
  with Cnvs do
  begin
    if daMonth in FDrawElement then
    begin
...
begin
  ...
  then
    begin
      ...
      TempDate := EncodeDate(AYear, AMonth, ADay);
      Date := TempDate;
      if Assigned(FOnNumberClick) and (daDay in FDrawElement) then FOnNumberClick(Self);
    end;
end;
