...
begin
  ...
  UndoCmd.Enabled := (FUndoBuffer.UndoCount > 1);
...
begin
  Result := frxConfirmMsg('Cохранить изменения?', mb_YesNoCancel);
end;
...
begin
{$IFDEF FR_LITE}
  Caption := Application.Title + ' - ' + GetReportName;
{$ELSE}
  Caption := Application.Title + ' - ' + GetReportName;
{$ENDIF}
end;
...
{$IFDEF FR_LITE}
  AboutCmd.Caption := StringReplace(frxGet(2403), Application.Title, Application.Title, []);
{$ELSE}
